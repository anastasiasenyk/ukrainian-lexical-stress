    ...
    if cfg.do_testing:
        ...
        if g2p_model is None:
            if os.path.exists(cfg.pretrained_model):
                ...
                model_cfg = G2PModel.restore_from(
                    restore_path=cfg.pretrained_model, return_config=True
                )
                ...
                g2p_model = imported_class.restore_from(
                    restore_path=cfg.pretrained_model, map_location=map_location
                )
                model_name = os.path.splitext(os.path.basename(cfg.pretrained_model))[0]
                logging.info(
                    f"Restored {model_name} g2p_model from {cfg.pretrained_model}."
                )
            elif cfg.pretrained_model in G2PModel.get_available_model_names():
                # restore g2p_model by name
                g2p_model = G2PModel.from_pretrained(
